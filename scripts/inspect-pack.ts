...
const temporaryRoot = mkdtempSync(join(tmpdir(), 'react-native-sf-symbol-pack-'))
const packDirectory = join(temporaryRoot, 'pack')
const extractDirectory = join(temporaryRoot, 'extract')
const npmCache = join(temporaryRoot, 'npm-cache')

const run = (command: string[], cwd = packageRoot): string => {
...
const isAllowedPath = (path: string): boolean =>
  path === 'LICENSE' ||
  path === 'README.md' ||
  path === 'ReactNativeSFSymbol.podspec' ||
...
try {
  ...
  const unexpectedFiles = packResult.files.map(file => file.path).filter(path => !isAllowedPath(path))
  if (unexpectedFiles.length > 0) throw new Error(`Unexpected tarball files:\n${unexpectedFiles.join('\n')}`)

  const requiredFiles = ['LICENSE', 'README.md', 'ReactNativeSFSymbol.podspec', 'build/index.d.ts', 'build/index.js']
